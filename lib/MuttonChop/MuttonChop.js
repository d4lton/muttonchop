...
MuttonChop.prototype.print = function(key) {
  //this.code += 'html += this.getDottedKeyValue(\'' + key + '\', \'' + JSON.stringify(this.data) + '\');\n';
  this.code += 'html += ' + key + ';\n';
}

MuttonChop.prototype.getDottedKeyValue = function(key, dataJson) {
  var data = JSON.parse(dataJson);
  var keyParts = key.split('.');
  var value = data;
  keyParts.forEach(function(keyPart) {
    if (value) {value = value[keyPart];}
  });
  return value;
}

MuttonChop.prototype.getDottedValue = function(key, dataJson) {
}
